...
const WebsocketProvider = ({ children }: { children: ReactNode }) => {
  ...
  const serverIsRunningOnProcessorHardware =
    useServerIsRunningOnProcessorHardware();
  ...
  useEffect(() => {
    async function joinWebsocket() {
      ...
      const url = `${wsPath}/ui/join/${token}?clientId=${clientId}`;
      ...
      newWs.onclose = (closeEvent: CloseEvent): void => {
        ...
        if (closeEvent.code === 4100) {
          // Code used in clientRef.current.close() in useEffect cleanup
