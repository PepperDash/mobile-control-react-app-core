...
- **Call `setSyncStateRequested()` once, after the loop, not inside it.** Both examples above do this correctly. Calling it inside `deviceKeysSet.forEach` is harmless when the set is non-empty (it just sets the same guard repeatedly) but means the guard is never set at all if `deviceKeysSet` ends up empty — the Wrapper would then recompute and pointlessly re-check every re-render. Always call it once after the loop finishes.
